"""
XGBoost model for body composition prediction.
"""
...
class XGBoostModel:
    """Multi-output XGBoost for BodyFat% and MuscleMass prediction."""
    ...
    def print_metrics(self, dataset_name="Test"):
        ...
        print(f"\n{'='*60}")
        print(f"XGBoost - {dataset_name} Set Metrics")
        print(f"{'='*60}")
    ...
    def plot_predictions(self, X, y, save_path=None):
        ...
        for i, (ax, target_name) in enumerate(zip(axes, target_names)):
            ...
            ax.set_title(f'{target_name} - XGBoost')
            ax.legend()
...
def train_xgboost(X_train, X_test, y_train, y_test, feature_names=None, tune=False, save_dir=None):
    """
    ...
    if save_dir:
        ...
        model.save(save_dir / 'xgboost_model.pkl')
        model.plot_predictions(X_test, y_test, save_dir / 'xgboost_predictions.png')
        
        if feature_names:
            model.plot_feature_importance(feature_names, save_dir / 'xgboost_feature_importance.png')
        ...
        metrics_df.to_csv(save_dir / 'xgboost_metrics.csv', index=False)
        
        # Save feature importance
        if feature_names:
            importances = model.get_feature_importance(feature_names)
            for target, imp_df in importances.items():
                filename = f'xgboost_importance_{target.replace(" ", "_")}.csv'
